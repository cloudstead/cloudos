...
#
# The "prep.sh" script (which calls prep-deploy.sh for each deployable component) expects
# that any output line starting with "ARTIFACT: " is the path to an artifact. prep.sh will then
# copy these to the (local or remote) destination.
#
# You can customize how a particular artifact-type builds its artifacts in two ways:
#
# 1. Local prep-deploy.sh script
# You can perform additional prep-deployment processing here.
# This file is required; its presence identifies the directory containing it as an artifact-type
#
# 2. Local prep-deploy.env
# If prep-deploy.env is present in the same directory as the local prep-deploy.sh script,
# it may define the following environment variables:
#
...
if [ ${IS_SERVER} -eq 1 ] ; then
    ...
    if [ ! -z "${PD_SKIP_LOCAL_SITE}" ] ; then
        echo "${ARTIFACT}: skipping 'site' creation"
    ...
    if [[ -x gen-sql.sh && ! -z "${GEN_SQL}" ]] ; then
      ...
      if [ $? -ne 0 ] ; then
          die "Error running gen-sql.sh: check ${GEN_SQL_LOG}"
      fi
      rm -f ${GEN_SQL_LOG}
    fi
...
if [ -f ${ARTIFACT_DIR}/prep-deploy.sh ] ; then
  ...
  die "No deployable artifacts found in ${ARTIFACT_DIR}"
fi
